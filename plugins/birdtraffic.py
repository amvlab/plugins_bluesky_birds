...
class BirdTraffic(core.Entity):
    ...
    def update(self):
        ''' to update bird positions either from a scenario or individually created'''
...
######################################################################################################
    ...
    def crescenario(self,filename):
        ...
        # give the user the choice to pass a scenario with or without header
        data = pd.read_csv(filename2read, engine= 'python', index_col= False)
        
        if not 'no_individuals' in data.columns:
            data.columns = ['id', 'time', 'lon','lat', 'alt', 'cat', 'no_individuals', 'flock_flag',
                            'id1', 'trk', 'spd']
...
# --> move birds from scenario files according to their recorded positions and 
...
######################################################################################################        
    ...
    def create_individual(self, birdid, birdtype, flock_flag, bird_size, no_inds, birdlat, birdlon, birdtrk, birdalt, birdspd):
        ...
        # placeholders only (required for scenario birds)
...
##############
    ...
    def update_position_scen(self):
        ...
        if np.any(altchange != 0.0):
            ...
            self.alt[idx4altchange] = (self.next_alt[idx4altchange] - self.last_alt[idx4altchange])*(delta_time_now[idx4altchange]/entire_delta_t[idx4altchange]) + self.last_alt[idx4altchange] 
    ...
    def update_position_individuals(self):
        ...
        # since last position
        self.lat, self.lon = self.extrapolate_position(delta_time_now, delta_distance, self.lat, self.lon, self.trk)
...
######################################################################################################   
    ...
    def CREBIRD(self, birdid, birdtype: str="pelican", bird_size: int = 3, no_inds: int = 1, birdlat: float=52., birdlon: float=4., birdtrk: float=None, birdalt: float=0,  
            birdspd: float = 0):
        ...
        # create the bird
        self.create_individual(birdid, birdtype, flock_flag, bird_size, no_inds, birdlat, birdlon, birdtrk, birdalt, birdspd)
...
######################################################################################################      
# Leo's positions - calculate the last position of weather radar birds
###################################################################################################### 


    def extrapolate_position(self, delta_t, delta_s, lat_in, lon_in, trk):
...
############################
#CHECK BIRDS tests whether the current birds are known already. If not, they are created 
############################
    ...
    def check_bird(self, input_id1, input_bird_size, input_no_inds, input_flock_flag, input_alt):
        ...
        # small
        add_radius[add_bird_size == 1] = (np.sqrt(add_no_inds[add_bird_size == 1])* 0.5 * 0.32) + 0.06
        # medium
        add_radius[add_bird_size == 2] = (np.sqrt(add_no_inds[add_bird_size == 2])* 0.5 * 0.68) + 0.16
        # flock flock
        add_radius[add_bird_size == 3] = (np.sqrt(add_no_inds[add_bird_size == 3])* 0.5 * 1.40) + 0.41
...
############
# REMOVE INPUT which was already processed
############
