...
class BirdTraffic(glh.RenderObject, layer=100):
    ...
    def create(self):
        ...
        self.bird_symbol.set_attribs(
            lat=self.bird_lat,
            lon=self.bird_lon,
            color=self.bird_color,
            orientation=self.bird_trk,
            instance_divisor=1,
        )

        self.birdlabels.create(
            self.bird_lbl,
            self.bird_lat,
            self.bird_lon,
            self.bird_color,
            (bird_size, -0.5 * bird_size),
            instanced=True,
        )
    ...
    def update_bird_data(self, data):
        ...
        # colors
        rawlabel = ""
        color = np.empty((min(self.nbirds, MAX_NBIRDS), 4), dtype=np.uint8)
        ...
        for i, (id, b_type, alt, vs, gs, angry_bird) in enumerate(zdata):
            ...
            if self.show_lbl:
                ...
                rawlabel += "%1s  %-8d" % (chr(vsarrow), int(gs / kts + 0.5))
